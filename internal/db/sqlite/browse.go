...
// buildWhereClause converts filters to a SQL WHERE clause and bound args.
func buildWhereClause(d SQLite, filters []db.Filter) (string, []any) {
	if len(filters) == 0 {
		return "", nil
	}
	var conds []string
...
func (s SQLite) FetchTableRows(ctx context.Context, sqlDB *sql.DB, o db.RowsOpts) (db.RowsPage, error) {
	if o.Page < 1 {
		o.Page = 1
	}
	if o.PerPage < 1 {
	...
	qualified := s.QuoteIdent(o.Table)
	if o.Schema != "" && o.Schema != "main" {
		qualified = o.Schema + "." + qualified
	}

	whereClause, whereArgs := buildWhereClause(s, o.Filters)
	...
	if o.SortCol != "" {
		...
		orderBy = " ORDER BY " + s.QuoteIdent(o.SortCol) + " " + dir
	}
