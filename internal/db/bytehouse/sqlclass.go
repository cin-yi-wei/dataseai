...
// stripComments removes /* ... */ block comments and -- line comments while
// preserving content inside string literals and backtick-quoted identifiers.
...
// identRE matches a ClickHouse/ByteHouse identifier: backtick-quoted or bare.
...
// ClassifySQL classifies a ByteHouse/ClickHouse SQL statement.
func (BH) ClassifySQL(sql string) (db.Classified, error) {
	...
	case "CREATE", "DROP", "GRANT", "REVOKE", "DENY", "ATTACH", "DETACH":
		c.Op = db.OpForbidden
	case "SHOW", "DESCRIBE", "EXPLAIN", "SET", "USE", "OPTIMIZE",
		"SYSTEM", "KILL", "CHECK", "EXISTS":
		c.Op = db.OpReadMeta
	default:
		c.Op = db.OpUnknown
	}
