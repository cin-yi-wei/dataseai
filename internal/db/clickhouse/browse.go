...
func (c CH) FetchTableRows(ctx context.Context, sqlDB *sql.DB, o db.RowsOpts) (db.RowsPage, error) {
	if o.Page < 1 {
		o.Page = 1
	}
	if o.PerPage < 1 {
	...
	qualified := c.QuoteIdent(o.Schema) + "." + c.QuoteIdent(o.Table)
	whereClause, whereArgs := buildWhereClause(c, o.Filters)
	...
	if o.SortCol != "" {
		...
		orderBy = " ORDER BY " + c.QuoteIdent(o.SortCol) + " " + dir
	}

	queryArgs := append([]any{}, whereArgs...)
	queryArgs = append(queryArgs, o.PerPage, offset)
	rows, err := sqlDB.QueryContext(ctx,
		fmt.Sprintf("SELECT * FROM %s%s%s LIMIT ? OFFSET ?", qualified, whereClause, orderBy),
		queryArgs...,
	)
	if err != nil {
		return db.RowsPage{}, err
...
func buildWhereClause(c CH, filters []db.Filter) (string, []any) {
	if len(filters) == 0 {
		return "", nil
	}
	var conds []string
	...
	for _, f := range filters {
		...
		case "IN", "NOT IN":
			...
			phs = "(" + phs[:len(phs)-2] + ")"
			conds = append(conds, col+" "+f.Operator+" "+phs)
			for _, p := range parts {
				args = append(args, p)
			}
		case "BETWEEN", "NOT BETWEEN":
			...
			conds = append(conds, fmt.Sprintf("%s %s ? AND ?", col, f.Operator))
