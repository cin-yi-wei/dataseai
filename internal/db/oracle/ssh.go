...
func (Oracle) RegisterSSHDialer(cfg db.SSHConfig, in db.DSNInput) (string, func(), error) {
	if cfg.IsZero() {
	...
	key := oraSSHReg.nextKey()
	oraSSHReg.put(key, &oraSSHEntry{client: client, listener: listener})
	...
	return localAddr, func() { closeOraSSH(key) }, nil
}
...
type oraSSHEntry struct {
	client   *gossh.Client
	listener net.Listener
}

type oraSSHRegistry struct {
	mu sync.Mutex
	m  map[string]*oraSSHEntry
	n  uint64
}

func (r *oraSSHRegistry) put(key string, e *oraSSHEntry) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.m[key] = e
}

func (r *oraSSHRegistry) take(key string) *oraSSHEntry {
	r.mu.Lock()
	defer r.mu.Unlock()
	e := r.m[key]
	delete(r.m, key)
...
func (r *oraSSHRegistry) nextKey() string {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.n++
...
var oraSSHReg = &oraSSHRegistry{m: map[string]*oraSSHEntry{}}

func closeOraSSH(key string) {
	e := oraSSHReg.take(key)
	if e == nil {
		return
	}
	_ = e.listener.Close()
