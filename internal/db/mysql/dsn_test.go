...
func TestBuildDSN(t *testing.T) {
	cases := []struct {
		...
		{"simple", db.DSNInput{Host: "h", Port: 3306, Username: "u", Password: "p", TLS: "disabled"}, "false"},
		{"awkward pw", db.DSNInput{Host: "h", Port: 3307, Username: "u", Password: "p:@/", DefaultDB: "mydb", TLS: "required"}, "true"},
		{"preferred", db.DSNInput{Host: "h", Port: 3306, Username: "u", Password: "p", TLS: "preferred"}, "preferred"},
		{"skip-verify", db.DSNInput{Host: "h", Port: 3306, Username: "u", Password: "p", TLS: "skip-verify"}, "skip-verify"},
		{"slash in pw (sandbox_ssh repro)", db.DSNInput{Host: "h", Port: 3306, Username: "fatgame", Password: "vup2u/3yj04", TLS: "disabled"}, "false"},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			dsn := MySQL{}.BuildDSN(c.in)
